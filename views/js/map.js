...
map.on("drag", () => map.panInsideBounds(L.latLngBounds(L.latLng(-85, -180), L.latLng(85, 180))));
map.on("click", () => {
	if (focusedMarker) {
...
document.getElementById("search-field").addEventListener(
	...
	debounce((e) => {
		...
		if (!query) {
			sidebar_tabs.querySelectorAll(".label, .content").forEach((el) => el.classList.remove("filtered-out"));
			return;
		}

		sidebar_tabs.querySelectorAll(".label, .content").forEach((el) => el.classList.add("filtered-out"));
		...
		const showParents = (element) => {
			...
			while (current && current !== sidebar_tabs) {
				...
				if (current.classList.contains("label")) {
					...
					if (nextContent?.classList.contains("content")) nextContent.classList.remove("filtered-out");
				}

				const parent = current.parentElement;
				const parentLabel = parent?.previousElementSibling;
				if (parentLabel?.classList.contains("label")) parentLabel.classList.remove("filtered-out");
...
sidebar_button.addEventListener(
	"click",
	() =>
		(sidebar_button.querySelector("img").src = `/assets/icons/${
			sidebar.classList.toggle("sidebar-open") ? "close" : "open"
		}-sidebar.png`)
);
				...
				layer.on({
					click: function () {
						...
						const toggleOutOfBounds = (region, isOut) => {
							...
							if (label?.classList.contains("label")) label.classList.toggle("out-of-bounds", isOut);
...
async function receiveCamCard(ip, port) {
	try {
		if (!validators.isValidIP(ip) || !validators.isValidPort(port)) throw new Error("Invalid IP or port");

		const response = await api.get(`/cam/${encodeURIComponent(ip)}/${encodeURIComponent(port)}`);
...
function renderImageViewer(event) {
	...
	slider.childNodes.forEach((image_elem) => {
		...
		if (image_elem.src === pressed_image.src) image_elem.classList.add("active");
	});
...
function changeActiveImage(direction) {
	...
	let new_active = null;
	if (direction === "left") new_active = active_image.previousElementSibling ?? slider.children[images_count - 1];
	else if (direction === "right") new_active = active_image.nextElementSibling ?? slider.children[0];
